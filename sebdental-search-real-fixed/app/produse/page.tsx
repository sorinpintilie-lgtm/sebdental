...
export default async function ProdusePage({
  ...
  if (sort === "relevanta") filtered = [...filtered].sort((a, b) => scoreProductSearch(b, q) - scoreProductSearch(a, q));
  ...
  return (
    <section className="grid gap-8 lg:grid-cols-[320px_1fr]">
      ...
      <div>
        ...
        <div className="mt-4 text-sm text-fg/65">
          {normalizedQ ? `${filtered.length} rezultate pentru „${q}”` : `${filtered.length} produse afișate`}
        </div>
        {filtered.length > 0 ? (
